...
const RadioButton = ({ name, value, label, checked, onChange }) => (
    <label className={`relative flex items-center justify-center p-3.5 rounded-xl border cursor-pointer transition-all duration-300 flex-1 
        ...
        {checked && (
            <div className="absolute top-2 right-2 w-1.5 h-1.5 bg-red-500 rounded-full shadow-[0_0_5px_#ef4444]" />
        )}
    </label>
);

const SectionHeader = ({ title, icon }) => (
    <div className="flex items-center gap-4 mb-8 pt-8 border-t border-white/5 first:border-0 first:pt-0">
        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-red-600 to-[#800000] flex items-center justify-center text-white shadow-lg shadow-red-900/40 border border-white/10">
            {icon}
        </div>
        <div className="flex flex-col">
            <h3 className="text-lg font-black text-white uppercase tracking-tight leading-none">{title}</h3>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Fill in the details below</span>
        </div>
    </div>
);
    ...
    return (
        <div className="flex flex-col md:col-span-2 group">
            ...
            <div className={`relative flex items-center gap-4 p-4 rounded-xl border border-dashed transition-all duration-300
                ${preview ? 'border-emerald-500/50 bg-emerald-900/10' : 'border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/30'}`}>
                ...
                {!preview ? (
                    <label className={`flex-1 flex flex-col md:flex-row items-center justify-center gap-3 py-4 cursor-pointer transition-all
                        ...
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors 
                            ${uploading ? 'bg-slate-700' : 'bg-white/5 border border-white/10 text-red-400'}`}>
                            {uploading ? (
                    ...
                    <div className="flex-1 flex items-center justify-between min-w-0">
                        <div className="flex items-center gap-4 overflow-hidden">
                            <div className="w-10 h-10 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 shadow-sm border border-emerald-500/20">
                                {Icons.check}
                            </div>
                            <div className="flex flex-col min-w-0">
                                <span className="text-xs font-bold text-emerald-400 truncate">Upload Complete</span>
                                <a href={preview} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[10px] font-bold text-slate-400 hover:text-white hover:underline transition-colors mt-0.5">
                        ...
                        <button 
                            ...
                            className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 text-red-400 hover:bg-red-500 hover:text-white transition-all shadow-sm"
                            title="Remove File"
                        >
                            {Icons.trash}
                        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="col-span-1">
                ...
                <div className="relative group">
                    ...
                    <div className="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-slate-500 text-xs group-hover:text-white">▼</div>
                </div>
            </div>
            ...
            <div className="col-span-1 lg:col-span-1">
                ...
                <div className="relative group">
                    <select name="gradeLevel" value={data.gradeLevel} onChange={handleChange} required className="w-full font-bold text-white bg-white/5 border-2 border-amber-500/50 rounded-xl p-4 focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 outline-none appearance-none shadow-sm cursor-pointer transition-all">
                        ...
                        <option value="Pre-Kindergarten 1" className="text-slate-900">Pre-Kindergarten 1</option>
                        <option value="Pre-Kindergarten 2" className="text-slate-900">Pre-Kindergarten 2</option>
                        <option value="Kinder" className="text-slate-900">Kinder</option>
                        {['Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6', 'Grade 7', 'Grade 8', 'Grade 9', 'Grade 10'].map(g => (<option key={g} value={g} className="text-slate-900">{g}</option>))}
                        <option value="Grade 11 (SHS)" className="text-slate-900">Grade 11 (SHS)</option>
                        <option value="Grade 12 (SHS)" className="text-slate-900">Grade 12 (SHS)</option>
                    </select>
                    <div className="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-amber-500 text-xs">▼</div>
                </div>
                {ageRule && !ageError && <div className="mt-3 text-[10px] text-amber-200 font-bold bg-amber-900/20 p-2.5 rounded-lg border border-amber-500/30 flex items-center gap-2"><span>💡</span> {ageRule}</div>}
                {ageError && <div className="mt-3 text-[10px] text-red-300 font-bold bg-red-900/20 p-2.5 rounded-lg border border-red-500/30 flex items-center gap-2 animate-pulse"><span>🚫</span> Requirement not met</div>}
            </div>
            ...
            <div className="col-span-1">
                ...
                <div className="flex gap-2 bg-white/5 p-1.5 rounded-xl border border-white/5">
                    {['With LRN', 'No LRN'].map((status) => (
                        <label key={status} className={`flex-1 cursor-pointer text-center text-[10px] font-bold uppercase py-3 rounded-lg transition-all 
                            ...
                            {data.psaScanUrl2 || showPage2 ? (
                                ...
                                <div>
                                    <button 
                                        ...
                                        className="text-[10px] font-bold text-red-400 flex items-center gap-2 hover:underline opacity-80 hover:opacity-100 transition-all bg-white/5 px-4 py-2 rounded-lg border border-dashed border-red-500/30 hover:border-red-500"
                                    >
                ...
                <div className="md:col-span-4 bg-gradient-to-r from-white/5 to-transparent rounded-xl p-5 border border-white/5 flex flex-col md:flex-row md:items-center gap-6">
                    ...
                    <div className="flex gap-4">
                        <label className="flex items-center gap-2 cursor-pointer group">
                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${!data.isIP ? 'border-red-500' : 'border-slate-600'}`}>
                            ...
                            <span className="text-xs font-bold uppercase text-white">No</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer group">
                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${data.isIP ? 'border-red-500' : 'border-slate-600'}`}>
                                {data.isIP && <div className="w-2.5 h-2.5 bg-red-500 rounded-full" />}
                            ...
                            <span className="text-xs font-bold uppercase text-white">Yes</span>
                        </label>
                    </div>
                    {data.isIP && (
                        <input 
                            ...
                            className="flex-1 bg-transparent border-b-2 border-slate-600 focus:border-red-500 outline-none px-2 py-1 text-sm font-bold text-red-400 uppercase placeholder:text-slate-600 animate-fade-in" 
                        />
                    )}
                </div>
            </div>
...
const PreviousSchoolInfo = ({ data, handleChange, show }) => {
    if (!show) return null;
    return (
        <div className="bg-blue-900/10 rounded-2xl p-6 md:p-8 border border-blue-500/20 mb-10 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-blue-500/10">
...
const SHSDetails = ({ data, handleChange, isTrackDisabled, showStrand }) => {
    ...
    return (
        <div className="bg-amber-900/10 rounded-2xl p-6 md:p-8 border border-amber-500/30 relative overflow-hidden mb-8 animate-fade-in shadow-lg shadow-amber-900/20">
             ...
             <div className="relative z-10">
                <h3 className="text-xl font-black text-amber-400 uppercase tracking-tight mb-8 flex items-center gap-3">
    ...
    if (submitted) {
        return (
            <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 relative overflow-hidden">
                <div className="fixed inset-0 z-0"><div className="absolute inset-0 bg-slate-950" /><img src="/2.png" alt="bg" className="absolute inset-0 w-full h-full object-cover opacity-40" /><div className="absolute inset-0 bg-gradient-to-b from-slate-950/90 via-slate-950/60 to-slate-950/90" /></div>
                
                <div className="relative z-10 bg-slate-900/90 backdrop-blur-xl p-8 md:p-12 rounded-[2rem] shadow-2xl text-center max-w-md w-full border border-white/10 animate-fade-in-up">
                    <div className="w-20 h-20 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-6 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.2)] border border-emerald-500/20">
                        {Icons.check}
                    </div>
                    <h2 className="text-2xl font-black text-white mb-3">{existingId ? 'Updated Successfully!' : 'Pre-Enrolled Successfully!'}</h2>
                    
                    <div className="text-left text-sm text-slate-400 leading-relaxed space-y-4 mb-8 bg-black/20 p-6 rounded-2xl border border-white/5">
                        <p>You have successfully <strong className="text-white">pre-enrolled</strong> to the San Ramon Catholic School, Inc.</p> 
                        <p className="font-bold text-red-400 bg-red-900/20 p-3 rounded-xl border border-red-500/20 text-xs">
                           ⚠ IMPORTANT: Visit the school in person to submit these requirements on Enrollment Day:
                        </p>
                        
                        <ul className="list-disc pl-5 text-[11px] text-slate-500 space-y-1 font-medium">
                    ...
                    <Link to="/enrollment-landing" className="w-full block bg-gradient-to-r from-[#800000] to-red-700 text-white font-bold py-4 rounded-xl hover:to-red-600 transition-all shadow-lg shadow-red-900/40 hover:-translate-y-1">
                        Back to Home
                    </Link>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[#020617] font-sans text-slate-300 pb-20 relative selection:bg-red-500 selection:text-white">
            ...
            <div className="fixed inset-0 z-0 pointer-events-none">
                ...
                <img src="/2.png" alt="Portal Background" className="absolute inset-0 w-full h-full object-cover opacity-50" />
                ...
                <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                        <div className="bg-white/10 backdrop-blur-md p-3 rounded-2xl shadow-2xl border border-white/20">
                            <img src="/1.png" alt="Logo" className="w-14 h-14 object-contain drop-shadow-md" />
                        </div>
                        <div className="text-white">
                            <h1 className="text-2xl md:text-3xl font-black tracking-tight leading-none drop-shadow-lg uppercase">Enrollment Form</h1>
                            <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] md:text-xs mt-1">San Ramon Catholic School, Inc.</p>
                        </div>
                    </div>
                    <Link to="/enrollment-landing" className="px-6 py-3 bg-white/5 backdrop-blur-md border border-white/10 rounded-full text-slate-300 text-xs font-bold uppercase tracking-wider hover:bg-white/10 hover:text-white transition-all flex items-center gap-2 group">
...
const ReviewModal = ({ data, onCancel, onConfirm, loading, isUpdateMode }) => {
    const DataRow = ({ label, value }) => (
        <div className="flex flex-col sm:flex-row sm:justify-between border-b border-white/5 py-3 last:border-0 hover:bg-white/5 px-2 rounded-lg transition-colors">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 sm:mb-0">{label}</span>
            <span className="text-sm font-bold text-white text-right break-words">{value || <span className="text-slate-600 font-normal italic">N/A</span>}</span>
        </div>
    );

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-fade-in">
            <div className="bg-slate-900 rounded-[2rem] w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl border border-white/10 overflow-hidden">
                {/* Modal Header */}
                <div className="p-6 bg-slate-900 border-b border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-[#800000] text-white rounded-full flex items-center justify-center shadow-lg shadow-red-900/50">
                            {isUpdateMode ? '🔄' : '📋'}
                        </div>
                        <div>
                            <h2 className="text-lg font-black text-white uppercase tracking-tight">{isUpdateMode ? 'Update Record' : 'Confirm Enrollment'}</h2>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Verify Details</p>
                        </div>
                    </div>
                    <button onClick={onCancel} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-slate-400 hover:bg-white/10 hover:text-white transition-colors">✕</button>
                </div>
                ...
                <div className="p-8 overflow-y-auto space-y-8 custom-scrollbar bg-slate-900/50">
                    <div className="space-y-6">
                        ...
                        <div>
                            <h4 className="text-slate-200 font-bold uppercase text-[10px] tracking-widest mb-4 border-b border-white/5 pb-2">Student Profile</h4>
                            <div className="flex justify-center mb-6">
                                {data.studentPhotoUrl ? (
                                    <img src={data.studentPhotoUrl} alt="Student" className="w-24 h-24 object-cover rounded-full border-4 border-slate-800 shadow-2xl shadow-black/50" />
                                ) : (
                                    <div className="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center text-xs text-slate-600 border-4 border-slate-800 shadow-inner">No Photo</div>
                                )}
                            </div>
                        ...
                        <div>
                            <h4 className="text-slate-200 font-bold uppercase text-[10px] tracking-widest mb-4 border-b border-white/5 pb-2">Contacts</h4>
                ...
                {/* Footer */}
                <div className="p-6 border-t border-white/5 bg-slate-900 flex gap-4">
                    <button onClick={onCancel} className="flex-1 py-4 rounded-xl font-bold text-slate-400 bg-white/5 border border-white/5 hover:bg-white/10 hover:text-white transition-colors">
                        Back to Edit
                    </button>
                    <button onClick={onConfirm} disabled={loading} className="flex-1 py-4 rounded-xl font-bold text-white bg-gradient-to-r from-[#800000] to-red-600 hover:to-red-500 shadow-lg shadow-red-900/40 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5">
