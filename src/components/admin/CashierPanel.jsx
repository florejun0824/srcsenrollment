...
import { Search, RefreshCw, CreditCard, Calendar, FileText, Download, Users, Printer, Edit3, Save, X, CheckCircle, Wallet, Building2, Banknote, Ban, AlertTriangle, Trash2, Settings } from 'lucide-react';
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {['Cash', 'GCash', 'Bank Transfer'].map((mode) => {
                                            const isDisabled = mode === 'GCash' || mode === 'Bank Transfer';
                                            return (
                                                <button
                                                    key={mode}
                                                    type="button"
                                                    disabled={isDisabled}
                                                    onClick={() => !isDisabled && setFormData(prev => ({ ...prev, mode }))}
                                                    className={`px-2 py-3 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all ${
                                                        formData.mode === mode 
                                                        ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' 
                                                        : isDisabled 
                                                            ? 'bg-slate-50 border-slate-100 text-slate-300 cursor-not-allowed opacity-60' 
                                                            : 'bg-white border-slate-200 text-slate-500 hover:bg-indigo-50'
                                                    }`}
                                                >
        ...
        <div className="flex flex-col h-full bg-slate-50/50">
            ...
            <div className="p-6 pb-2">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    {/* INTEGRATION: FEE SETTINGS BUTTON */}
                    <button 
                        onClick={() => navigate('/admin/fees')} 
                        className="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-2 hover:bg-slate-50 transition-colors shadow-sm"
                    >
                        <Settings className="w-4 h-4" /> Fee Settings
                    </button>
                </div>
